...
// ZMIANA: Zwiekszono limit pradu, aby obsluzyc skoki 10A
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	...
	if (htim == &htim7) {
		...
		if (automationActive) {
			...
			// Definicja czasow w ms
			uint32_t onDuration = 10000;  // Czas trwania skoku (3 sekundy)
			uint32_t offDuration = 10000; // Czas przerwy (10 sekund)
			uint32_t period = onDuration + offDuration;

			// Tablica sekwencji pradow (8 krokow)
			float currentSteps[] = {
				3.0f,  0.0f,   // 2 skoki 5A
				0.0f, 0.0f,  // 2 skoki 10A
				0.0f, 0.0f,  // 2 skoki -5A
				0.0f, 0.0f // 2 skoki -10A
			};

			uint32_t stepIndex = automationCounter / period;
			uint32_t timeInStep = automationCounter % period;

			// Wykonaj sekwencje jesli index miesci sie w tablicy (0-7)
			if (stepIndex < 8) {
				if (timeInStep < onDuration) {
					// Stan wysoki (skok pradu)
					motor1Current = currentSteps[stepIndex];
					motor2Current = currentSteps[stepIndex];
				} else {
					// Stan niski (przerwa)
					motor1Current = 0.0f;
					motor2Current = 0.0f;
